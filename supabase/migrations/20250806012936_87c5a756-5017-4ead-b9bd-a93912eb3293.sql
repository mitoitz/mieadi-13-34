...
-- Atualizar CPFs vazios de forma sequencial
...
UPDATE profiles 
SET cpf = LPAD(numbered_rows.rn::text, 11, '0')
FROM numbered_rows 
WHERE profiles.id = numbered_rows.id;

-- Criar índice único para CPF se não existir
CREATE UNIQUE INDEX IF NOT EXISTS idx_profiles_cpf_unique ON profiles(cpf) WHERE cpf IS NOT NULL;
